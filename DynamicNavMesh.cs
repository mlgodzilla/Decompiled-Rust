...
public class DynamicNavMesh : SingletonComponent<DynamicNavMesh>, IServerComponent
{
  ...
  private void OnEnable()
  {
    ...
    ((FacepunchBehaviour) this).InvokeRepeating(new Action(this.FinishBuildingNavmesh), 0.0f, 1f);
  }
  ...
  private void OnDisable()
  {
    if (Application.isQuitting != null)
      return;
    ((FacepunchBehaviour) this).CancelInvoke(new Action(this.FinishBuildingNavmesh));
  ...
  {
    if (this.HasBuildOperationStarted || AiManager.nav_disable)
      return;
    float realtimeSinceStartup = Time.get_realtimeSinceStartup();
    Debug.Log((object) ("Starting Navmesh Build with " + (object) this.sources.Count + " sources"));
    NavMeshBuildSettings settingsByIndex = NavMesh.GetSettingsByIndex(this.NavMeshAgentTypeIndex);
  ...
  {
    DynamicNavMesh dynamicNavMesh = this;
    if (!dynamicNavMesh.HasBuildOperationStarted && !AiManager.nav_disable)
    {
      dynamicNavMesh.HasBuildOperationStarted = false;
      ((Bounds) ref dynamicNavMesh.Bounds).set_size(TerrainMeta.Size);
      NavMesh.set_pathfindingIterationsPerFrame(AiManager.pathfindingIterationsPerFrame);
      if (AiManager.nav_wait)
        yield return (object) dynamicNavMesh.CollectSourcesAsync(new Action(dynamicNavMesh.UpdateNavMeshAsync));
      else
        ((MonoBehaviour) dynamicNavMesh).StartCoroutine(dynamicNavMesh.CollectSourcesAsync(new Action(dynamicNavMesh.UpdateNavMeshAsync)));
      if (!AiManager.nav_wait)
      {
      ...
      {
        int lastPct = 0;
        while (!dynamicNavMesh.HasBuildOperationStarted)
        {
          Thread.Sleep(250);
          yield return (object) null;
        }
        while (dynamicNavMesh.BuildingOperation != null)
        {
          int num = (int) ((double) dynamicNavMesh.BuildingOperation.get_progress() * 100.0);
          if (lastPct != num)
          {
          ...
          Thread.Sleep(250);
          dynamicNavMesh.FinishBuildingNavmesh();
          yield return (object) null;
        }
      }
    }
  ...
  public void FinishBuildingNavmesh()
  {
    ...
    Debug.Log((object) string.Format("Navmesh Build took {0:0.00} seconds", (object) this.BuildTimer.Elapsed.TotalSeconds));
