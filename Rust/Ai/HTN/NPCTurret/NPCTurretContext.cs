...
// Type: Rust.Ai.HTN.NPCTurret.NPCTurretContext
...
namespace Rust.Ai.HTN.NPCTurret
{
  ...
  public class NPCTurretContext : BaseNpcContext, IDisposable
  {
      ...
      if (this._worldState == null || this._worldState.Length != length)
      {
        ...
        if (NPCTurretContext._worldStateChanges == null)
        {
          ...
          for (int index = 0; index < length; ++index)
            NPCTurretContext._worldStateChanges[index] = new Stack<WorldStateInfo>(5);
        }
      }
    ...
    {
      int index = (int) fact;
      Stack<WorldStateInfo> worldStateChange = NPCTurretContext._worldStateChanges[index];
    ...
    {
      int index = (int) fact;
      if (NPCTurretContext._worldStateChanges[index].Count <= 0)
        return;
      NPCTurretContext._worldStateChanges[index].Pop();
    }
    ...
    public byte PeekFactChangeDuringPlanning(byte fact)
    {
      int index = (int) fact;
      if (NPCTurretContext._worldStateChanges[index].Count > 0)
        return (byte) NPCTurretContext._worldStateChanges[index].Peek().Value;
      return 0;
    }
