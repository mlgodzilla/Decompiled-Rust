...
// Type: Rust.Ai.HTN.ScientistAStar.ScientistAStarContext
  ...
  public class ScientistAStarContext : BaseNpcContext, IDisposable
  {
    ...
    public override Vector3 GetDirectionToMemoryOfPrimaryEnemyPlayerTarget()
    {
      ...
      if (Object.op_Inequality((Object) knownEnemyPlayer.PlayerInfo.Player, (Object) null))
      {
        ...
        float num = (float) (vector3_1.y - this.BodyPosition.y);
    ...
    public Vector3 BodyLocalPosition
    {
      get
      {
        return ((Component) this.Body).get_transform().get_localPosition();
      }
    }
      ...
      bool checkValueDiff = true)
    {
      ...
      if (!invokeChangedEvent)
        return;
      ScientistAStarContext.WorldStateChangedEvent stateChangedEvent = this.OnWorldStateChangedEvent;
    ...
    {
      int index = (int) fact;
      if (ScientistAStarContext._worldStateChanges[index].Count <= 0)
        return;
      ScientistAStarContext._worldStateChanges[index].Pop();
    }
    ...
    public byte PeekFactChangeDuringPlanning(byte fact)
    {
      int index = (int) fact;
      if (ScientistAStarContext._worldStateChanges[index].Count > 0)
        return (byte) ScientistAStarContext._worldStateChanges[index].Peek().Value;
      return 0;
    }
