...
// Type: Rust.Ai.HTN.Murderer.MurdererContext
  ...
  public class MurdererContext : BaseNpcContext, IDisposable
  {
      ...
      bool checkValueDiff = true)
    {
      ...
      if (!invokeChangedEvent)
        return;
      MurdererContext.WorldStateChangedEvent stateChangedEvent = this.OnWorldStateChangedEvent;
    ...
    {
      int index = (int) fact;
      Stack<WorldStateInfo> worldStateChange = MurdererContext._worldStateChanges[index];
    ...
    {
      int index = (int) fact;
      if (MurdererContext._worldStateChanges[index].Count <= 0)
        return;
      MurdererContext._worldStateChanges[index].Pop();
    }
    ...
    public byte PeekFactChangeDuringPlanning(byte fact)
    {
      int index = (int) fact;
      if (MurdererContext._worldStateChanges[index].Count > 0)
        return (byte) MurdererContext._worldStateChanges[index].Peek().Value;
      return 0;
    }
