...
// Type: Rust.Ai.HTN.Scientist.ScientistContext
  ...
  public class ScientistContext : BaseNpcContext, IDisposable
  {
      ...
      if (this._worldState == null || this._worldState.Length != length)
      {
        ...
        if (ScientistContext._worldStateChanges == null)
        {
          ...
          for (int index = 0; index < length; ++index)
            ScientistContext._worldStateChanges[index] = new Stack<WorldStateInfo>(5);
        }
      }
      ...
      bool checkValueDiff = true)
    {
      ...
      if (!invokeChangedEvent)
        return;
      ScientistContext.WorldStateChangedEvent stateChangedEvent = this.OnWorldStateChangedEvent;
    ...
    {
      int index = (int) fact;
      Stack<WorldStateInfo> worldStateChange = ScientistContext._worldStateChanges[index];
    ...
    {
      int index = (int) fact;
      if (ScientistContext._worldStateChanges[index].Count <= 0)
        return;
      ScientistContext._worldStateChanges[index].Pop();
    }
    ...
    public byte PeekFactChangeDuringPlanning(byte fact)
    {
      int index = (int) fact;
      if (ScientistContext._worldStateChanges[index].Count > 0)
        return (byte) ScientistContext._worldStateChanges[index].Peek().Value;
      return 0;
    }
