...
// Type: Rust.Ai.HTN.Scientist.Reasoners.MaintainCoverReasoner
...
namespace Rust.Ai.HTN.Scientist.Reasoners
{
  public class MaintainCoverReasoner : INpcReasoner
  {
    ...
    public void Tick(IHTNAgent npc, float deltaTime, float time)
    {
      ...
      if (npcContext == null || !npcContext.IsFact(Facts.MaintainCover) && !npcContext.IsFact(Facts.IsReloading) && !npcContext.IsFact(Facts.IsApplyingMedical) || (npcContext.ReservedCoverPoint != null && !npcContext.ReservedCoverPoint.IsCompromised && (!npcContext.IsFact(Facts.AtLocationCover) && (double) time - (double) npcContext.ReservedCoverTime < 0.800000011920929) || (!npcContext.IsFact(Facts.CanSeeEnemy) && (double) npcContext.Body.SecondsSinceAttacked - 1.0 > (double) time - (double) npcContext.ReservedCoverTime || !ScientistDomain.CanNavigateToCoverLocation.Try(CoverTactic.Retreat, npcContext))))
        return;
