...
// Type: Rust.Ai.HTN.Scientist.Reasoners.OrientationReasoner
  ...
  public class OrientationReasoner : INpcReasoner
  {
    ...
    public void Tick(IHTNAgent npc, float deltaTime, float time)
    {
      ...
      if (npc.IsDestroyed || htnPlayer.IsDead() || htnPlayer.IsWounded())
        ...
        npcOrientation = npcContext.GetFact(Rust.Ai.HTN.Scientist.Facts.IsSearching) <= (byte) 0 || npcContext.GetFact(Rust.Ai.HTN.Scientist.Facts.IsNavigating) != (byte) 0 ? (npcContext.GetFact(Rust.Ai.HTN.Scientist.Facts.IsIdle) <= (byte) 0 ? NpcOrientation.LastKnownPrimaryTargetLocation : (!npcContext.IsFact(Rust.Ai.HTN.Scientist.Facts.CanHearEnemy) ? NpcOrientation.Heading : NpcOrientation.AudibleTargetDirection)) : NpcOrientation.LookAround;
      else if (npcContext.IsFact(Rust.Ai.HTN.Scientist.Facts.CanHearEnemy))
